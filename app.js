...
// Audio Engine State — Dual YouTube IFrame API & HTML5 Audio
let audioEl = null;
let ytPlayer = null;
...
async function loadData() {
  ...
  const localSaved = localStorage.getItem("local_music_catalog");
  if (localSaved) {
    try {
      const parsedLocal = JSON.parse(localSaved);
      if (Array.isArray(parsedLocal) && parsedLocal.length > 0) {
        if (!loadedCatalog || parsedLocal.length > loadedCatalog.length) {
          loadedCatalog = parsedLocal;
        }
      }
    } catch (e) {}
  }
...
// Audio Controls via YouTube IFrame API & HTML5 Audio
async function playTrack(track) {
  ...
  const thumbUrl = track.thumbnail || (track.youtubeId ? `https://i.ytimg.com/vi/${track.youtubeId}/hqdefault.jpg` : 'photos/b.jpg');
  ...
  } else if (track.src || track.audioUrl) {
    if (audioEl) {
      audioEl.src = track.src || track.audioUrl;
...
function togglePlayPause() {
  ...
  if (currentTrack.youtubeId && ytPlayer && ytPlayer.getPlayerState) {
